...
Nft mata::cntnft::remove_epsilon(const Nft& aut, Symbol epsilon) {
...
Nft mata::cntnft::project_out(const Nft& nft, const utils::OrdVector<Level>& levels_to_project, const JumpMode jump_mode) {
...
Nft mata::cntnft::project_out(const Nft& nft, const Level level_to_project, const JumpMode jump_mode) {
    return project_out(nft, utils::OrdVector{ level_to_project }, jump_mode);
}

Nft mata::cntnft::project_to(const Nft& nft, const OrdVector<Level>& levels_to_project, const JumpMode jump_mode) {
...
Nft mata::cntnft::project_to(const Nft& nft, Level level_to_project, const JumpMode jump_mode) {
...
Nft Nft::apply(const nfa::Nfa& nfa, Level level_to_apply_on, JumpMode jump_mode) const {
    Nft nft_from_nfa{ cntnft::builder::create_from_nfa(nfa, this->num_of_levels) };
...
Nft mata::cntnft::insert_levels(const Nft& nft, const BoolVector& new_levels_mask, const JumpMode jump_mode) {
...
Nft mata::cntnft::insert_level(const Nft& nft, const Level new_level, const JumpMode jump_mode) {
...
Nft mata::cntnft::fragile_revert(const Nft& aut) {
    const size_t num_of_states{ aut.num_of_states() };
...
Nft mata::cntnft::simple_revert(const Nft& aut) {
    Nft result;
    result.clear();
...
Nft mata::cntnft::somewhat_simple_revert(const Nft& aut) {
    const size_t num_of_states{ aut.num_of_states() };
...
Nft mata::cntnft::revert(const Nft& aut) {
    return simple_revert(aut);
    //return fragile_revert(aut);
    //return somewhat_simple_revert(aut);
}

std::pair<Run, bool> mata::cntnft::Nft::get_word_for_path(const Run& run) const {
...
bool mata::cntnft::Nft::is_in_lang(const Run& run) const {
...
bool mata::cntnft::Nft::is_prfx_in_lang(const Run& run) const {
    StateSet current_post{ this->initial };
    for (const Symbol sym : run.word) {
...
Nft mata::cntnft::uni(const Nft &lhs, const Nft &rhs) {
    Nft union_nft{ lhs };
...
Nft& Nft::uni(const Nft& aut) {
    ...
    SparseSet<mata::cntnft::State> aut_final_copy = aut.final;
    SparseSet<mata::cntnft::State> aut_initial_copy = aut.initial;
...
Simlib::Util::BinaryRelation mata::cntnft::algorithms::compute_relation(const Nft& aut, const ParameterMap& params) {
...
Nft mata::cntnft::reduce(const Nft &aut, StateRenaming *state_renaming, const ParameterMap& params) {
    if (!haskey(params, "algorithm")) {
        throw std::runtime_error(std::to_string(__func__) +
                                 " requires setting the \"algorithm\" key in the \"params\" argument; "
                                 "received: " + std::to_string(params));
...
Run mata::cntnft::encode_word(const Alphabet* alphabet, const std::vector<std::string>& input) {
    return mata::nfa::encode_word(alphabet, input);
}

std::set<mata::Word> mata::cntnft::Nft::get_words(size_t max_length) const {
