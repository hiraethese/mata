...
// TODO: Move this method to cntnft.hh? It is something one might want to use (e.g. for union, inclusion, equivalence of DFAs).
Nft mata::cntnft::algorithms::product(const Nft& lhs, const Nft& rhs, const std::function<bool(State,State)>&& final_condition, ProductMap *product_map, const JumpMode jump_mode, const State lhs_first_aux_state, const State rhs_first_aux_state) {
